"""Run leave-one-major-lineage-out evaluation for the one-hot CNN.
...
from cnn_utils import ProteinDataset, bootstrap_auc_ci, set_seed
from cnn_cv_training import _train_one_fold, DRUG2GENES, N_EPOCHS, LR, BATCH_SIZE, DEVICE, SEED

# train contains all non-held-out lineaged isolates.
OUT_ROOT = PROTEIN_TASKS_DIR / 'data/latest/lineage_ood_all_train/cnn'


def _write_aggregate_summary(drug: str) -> None:
    """Rebuild the per-drug aggregate table from split-level summaries.

    We intentionally derive `all_lineage_summary.csv` from the held-out split
    summaries already written to disk. That keeps the aggregate file stable even
    if we later rerun one lineage, rerun a single held-out group for debugging,
    or invoke `--dry-run`, which should never clobber a complete result table.
    """
