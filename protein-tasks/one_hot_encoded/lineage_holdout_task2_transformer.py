...
from transformer_utils import ProteinDataset, bootstrap_auc_ci, set_seed, shap_per_residue
...
OUT_ROOT = PROTEIN_TASKS_DIR / 'data/latest/lineage_ood_all_train/transformer_task2'
...
def run_lineage_task2_for_drug(drug: str, heldout_lineage: str, k_vals=(1, 5, 10),
    ...
    for row in pr_rows:
        row.update({'heldout_lineage': str(heldout_lineage), 'model': 'transformer'})
...
def main() -> None:
    ...
    if all_pr:
        ...
        pd.DataFrame(all_pr).to_csv(OUT_ROOT / 'combined_lineage_task2_precision_recall_transformer.csv', index=False)
        print(f'[done] wrote {OUT_ROOT / "combined_lineage_task2_precision_recall_transformer.csv"}')
