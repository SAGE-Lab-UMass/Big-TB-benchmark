...
from cnn_utils import (
    _build_drug_df,          # same builder used in original code
    ProteinDataset,          # same dataset class
    shap_per_residue,        # same SHAP helper
    bootstrap_auc_ci,        # pooled CI
    one_sample_test_vs_half, # one-sample test vs 0.5
    set_seed                 # usual seed helper
)
...
# SHAP (per-fold) — same semantics as original
COMPUTE_SHAP    = True
BG_SIZE         = 100
EXPL_SAMPLES    = 200

# Keep the same DRUG2GENES mapping already used elsewhere
DRUG2GENES = {
    "rifampicin"  : ["rpoB"],
    "pyrazinamide": ["pncA"],
    "capreomycin" : ["tlyA"],
...
# Output roots aligned with the current CV consumers
OUT_ROOT = Path("data/latest/cross_val/cnn_cv_sig")  
CURVE_DIR = OUT_ROOT                                  # per-fold curves/preds live here
MODEL_DIR = Path("data/latest/results/prediction/cnn_models")
MODEL_DIR.mkdir(parents=True, exist_ok=True)
INTERP_DIR = OUT_ROOT / "interpretability" / "cnn"
...
def train_eval_model_cv(
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(np.arange(len(full_ds)), labels), start=1):
        ...
        # 3b) optional SHAP per fold (same helper & semantics)
        if compute_shap:
...
# ────────────────────────────────────────────────────────────────────
def run_cnn_cv_for_drug(drug: str, n_splits: int = N_SPLITS) -> pd.DataFrame:
    """
    Mirrors `run_cnn_for_drug`: builds df & per-gene lengths, then calls `train_eval_model_cv`.
    ...
    # Build df same way  original code does, then drop helpers
...
def main():
    ...
    for d in drug_list:
        try:
            summaries.append(run_cnn_cv_for_drug(d, n_splits=N_SPLITS))
