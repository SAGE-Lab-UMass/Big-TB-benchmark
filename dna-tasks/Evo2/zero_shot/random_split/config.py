...
PROJECT_DIR = Path(__file__).resolve().parents[2]  # Now two levels up to reach Evo2/

# Local downstream utilities (common to both zero-shot and finetuning).
LOCAL_DOWNSTREAM_MODULES_DIR = PROJECT_DIR / "downstream_modules"
LEGACY_FINETUNE_UTILS_DIR = PROJECT_DIR / "finetuning" / "modules"
LEGACY_FINETUNE_UTILS_DIR_V2 = PROJECT_DIR / "evo2_finetune_modules"
LEGACY_DNABERT_MODULES_DIR = PROJECT_DIR / "dnabert_modules"

# Backward-compatible resolver: prefer new folder name, then legacy folder names.
if LOCAL_DOWNSTREAM_MODULES_DIR.exists():
    FINETUNE_UTILS_DIR = LOCAL_DOWNSTREAM_MODULES_DIR
elif LEGACY_FINETUNE_UTILS_DIR.exists():
    ...
    FINETUNE_UTILS_DIR = LEGACY_DNABERT_MODULES_DIR
...
def ensure_finetune_utils_on_path() -> None:
    """Expose downstream utility modules (common to zero-shot and finetuning)."""
    if not FINETUNE_UTILS_DIR.exists():
        raise FileNotFoundError(
            "Missing downstream modules folder. Expected one of: "
            f"{LOCAL_DOWNSTREAM_MODULES_DIR}, {LEGACY_FINETUNE_UTILS_DIR}, "
            f"{LEGACY_FINETUNE_UTILS_DIR_V2}, or {LEGACY_DNABERT_MODULES_DIR}"
        )
