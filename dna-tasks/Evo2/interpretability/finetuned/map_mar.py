"""MAP/MAR helpers for finetuned Evo2 interpretability."""
...
def find_drug_column(df: pd.DataFrame, drug: str) -> str:
    for column in df.columns:
        if column.lower() == drug.lower():
            return column
    raise ValueError(f"Drug column {drug!r} not found")


def create_mutations_column(mapped_df: pd.DataFrame, has_neg_strand: bool) -> pd.DataFrame:
...
def _average_precision_at_k(predicted: Sequence[str], relevant: set[str], k: int) -> float:
    hits = 0
    precision_sum = 0.0
        ...
        rows.append(
            {
                ...
                "MAP@k": _average_precision_at_k(important_features_ranked, relevant_features, k),
                "Hits@k": hits,
            }
        )
    return rows
...
def mean_map_mar_rows(lineage_rows: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Cannot aggregate MAP/MAR rows; missing columns: {sorted(missing)}")
    return (
