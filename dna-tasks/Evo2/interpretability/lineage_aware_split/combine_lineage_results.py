"""Merge per-lineage MAP/MAR CSVs written by parallel array jobs.

When ``run_interpret_evo2_lineage_aware.py`` is invoked with
``--heldout-lineage`` (one process per eligible lineage, e.g. from a SLURM
array), each task writes its own
``map_mar/map_mar_<DRUG>_heldout_lineage_<N>.csv`` instead of the shared
``_by_lineage.csv`` / ``_mean.csv`` files. Run this script once every array
task has finished to combine them into the standard combined outputs.

Usage::

    python combine_lineage_results.py parameter_files/shap_interpret_lineage_aware.yaml --drug AMIKACIN
"""
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--drug", required=True, help="Drug whose per-lineage files should be combined")
    parser.add_argument(
        "--shard-count",
        type=int,
        default=1,
        help="Number of explainer shards expected for every lineage",
    )
    return parser.parse_args()


def combine_explainer_shards(
    config: dict,
    drug: str,
    shard_count: int,
) -> None:
    """Merge per-shard maxima into the exact full-explainer ranking."""
    if shard_count < 2:
        return
    ...
    for lineage_dir in lineage_dirs:
        ...
        missing = [
            str(path / "ranked_shap.csv")
            for path in shard_dirs
            if not (path / "ranked_shap.csv").is_file()
        ]
        if missing:
            raise FileNotFoundError(
                f"Missing {len(missing)} SHAP shard(s) for {drug} lineage {lineage}: "
                + ", ".join(missing)
            )
        ...
        for shard_dir in shard_dirs:
            ...
            background_parts.append(
                pd.read_csv(
                    shard_dir / "background_samples.csv",
                    dtype={"row_id": str, "sample_id": str},
                )
            )
            explainer_ids.extend(
                pd.read_csv(
                    shard_dir / "explainer_samples.csv", dtype={"sample_id": str}
                )["sample_id"].tolist()
            )
        ...
        if any(
            frame["sample_id"].tolist() != reference_background_ids
            for frame in background_parts[1:]
        ):
        ...
        if len(explainer_ids) != len(set(explainer_ids)) or set(explainer_ids) != fixed_ids:
            raise ValueError(
                f"Explainer shards do not form the fixed explainer for {drug} lineage {lineage}"
            )
        ...
        ranked_features = [
            f"{row.gene}_{int(row.position)}"
            for row in ranked.head(top_n_positions).itertuples()
        ]
        pd.DataFrame({"feature": ranked_features}).to_csv(
            lineage_dir / "selected_features.csv", index=False
        )
        background_parts[0].to_csv(lineage_dir / "background_samples.csv", index=False)

        selection = selection_parts[0].iloc[0].to_dict()
        selection["explainer_samples_in_shard"] = sum(
            int(frame.iloc[0]["explainer_samples_in_shard"])
            for frame in selection_parts
        )
        selection["explainer_shard_index"] = "all"
        selection["completed_shards"] = shard_count
        pd.DataFrame([selection]).to_csv(
            lineage_dir / "sample_selection_summary.csv", index=False
        )
        metric_rows = [
            {"drug": drug, "heldout_lineage": lineage, **row}
            for row in compute_map_mar_rows(ranked_features, relevant, k_values)
        ]
        pd.DataFrame(metric_rows).to_csv(
            output_dir / "map_mar" / f"map_mar_{drug}_heldout_lineage_{lineage}.csv",
            index=False,
        )
        shard_lineage_count += 1
        print(f"Combined {shard_count} SHAP shards for {drug} lineage {lineage}")

    if shard_lineage_count == 0:
        raise FileNotFoundError(f"No shard directories found below {drug_shap_dir}")
...
def main() -> None:
    ...
    if not per_lineage_paths:
        raise FileNotFoundError(
            f"No per-lineage MAP/MAR CSVs found for {drug} under {map_mar_dir}"
        )

    by_lineage = pd.concat((pd.read_csv(p) for p in per_lineage_paths), ignore_index=True)
    ...
    selection_paths = sorted(
        drug_shap_dir.glob("heldout_lineage_*/sample_selection_summary.csv")
    )
    if selection_paths:
        selection_summary = pd.concat(
            (pd.read_csv(path) for path in selection_paths), ignore_index=True
        ).sort_values("heldout_lineage")
        selection_summary.to_csv(
            drug_shap_dir / "sample_selection_summary.csv", index=False
        )

    print(f"Combined {len(per_lineage_paths)} lineage file(s) -> {by_lineage_path}")
