...
This is a minimal wrapper around ``zero_shot/random_split/train.py`` that:
...
4. Delegates all remaining training logic to the unchanged
   ``zero_shot/random_split/train.py`` pipeline.

The only change to ``zero_shot/random_split/`` code is that ``stratified_split_dataset``
...
# ── make zero_shot.random_split importable ────────────────────────────────────
THIS_DIR = Path(__file__).resolve().parent
EVO2_DIR = THIS_DIR.parents[1]
sys.path.insert(0, str(EVO2_DIR))
sys.path.insert(0, str(EVO2_DIR / "zero_shot" / "random_split"))

import zero_shot.random_split.train as evo2_train  # noqa: E402
from zero_shot.random_split.config import ensure_dnabert_transfer_learn_on_path  # noqa: E402

# lineage_split lives next to this file
sys.path.insert(0, str(THIS_DIR))
from utils.lineage_split import (  # noqa: E402
    MAJOR_LINEAGES,
    load_isolate_id_map,
    load_lineage_map,
