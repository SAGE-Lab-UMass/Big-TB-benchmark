...
class TokenMemmapMap(torch.utils.data.Dataset):
    def __init__(self, meta_paths, label_dict, max_open_memmaps=8):
        self.blocks = []
        self.lookup = []
        self.label_dict = label_dict
        self.max_open_memmaps = max_open_memmaps
        ...
        for bidx, meta_path in enumerate(meta_paths):
            ...
            self.blocks.append((ids, mmap_path, shape))
    ...
    def __getitem__(self, idx):
        ...
        ids, mmap_path, shape = self.blocks[bidx]
        mm = self._open_memmap(mmap_path, shape)
        seq_id = ids[ridx]
        x = torch.from_numpy(mm[ridx].astype("float32")).t()  # shape: (D, L)
        y = torch.tensor(self.label_dict[seq_id], dtype=torch.float32)
        return x, y

    def _open_memmap(self, mmap_path, shape):
        cache_key = (mmap_path, shape)
        if cache_key in self._mmap_cache:
            mm = self._mmap_cache.pop(cache_key)
            self._mmap_cache[cache_key] = mm
            return mm

        mm = np.memmap(mmap_path, dtype="float16", mode="r", shape=shape)
        self._mmap_cache[cache_key] = mm
        while len(self._mmap_cache) > self.max_open_memmaps:
            _, stale = self._mmap_cache.popitem(last=False)
            stale._mmap.close()
        return mm
...
class MultiGeneConcatDataset(torch.utils.data.Dataset):
    ...
    """
    def __init__(self, gene_dirs, label_map, max_open_memmaps=8):
        ...
        for gene_dir in gene_dirs:
            ...
            for meta_path in metas:
                ...
                for row, ident in enumerate(ids):
                    gene_locations[ident] = (mmap_path, shape, row)
            self.locations[gene] = gene_locations

        # Extract common identifiers across all genes
        id_sets = [set(self.locations[Path(g).name]) for g in gene_dirs]
    ...
    def _row(self, gene, ident):
        gene = Path(gene).name
        mmap_path, shape, row = self.locations[gene][ident]
        mm = self._open_memmap(mmap_path, shape)
        return torch.from_numpy(mm[row].astype("float32")).t()
