...
def find_hits_at_k(predicted, relevant, k, model_embed_type="tokens"):
    """
...
def compute_ap_at_k(predicted, relevant, k, model_embed_type="tokens"):
    hits = 0
    sum_precisions = 0
    matched = set()
...
def compute_ar_at_k(predicted, relevant, k, model_embed_type="tokens"):
    hits = find_hits_at_k(predicted, relevant, k, model_embed_type)
    # return hits / len(relevant) if relevant else 0.0
    return hits / min(k, len(relevant)) if relevant else 0.0

def compute_p_at_k(predicted, relevant, k, model_embed_type="tokens"):
    hits = find_hits_at_k(predicted, relevant, k, model_embed_type)
    return hits / k if relevant else 0.0

def compute_r_at_k(predicted, relevant, k, model_embed_type="tokens"):
    hits = find_hits_at_k(predicted, relevant, k, model_embed_type)
    return hits / len(relevant) if relevant else 0.0


def compute_map_mar_for_directory(directory, important_features_ranked, drug, has_neg_strand=False, embed_type='tokens'):
    ...
    export_relevant_set(relevant_set, f"/project/pi_annagreen_umass_edu/saishradha/project_data_curation/benchmarking/DNABERT_S/interpretability/output/shap_results/token_768/map_mar/relevant_features_{drug}.csv")
...
def get_confident_mutation_hits(
    ...
    output_csv = os.path.join(output_path, output_file_name)
