...
def calculate_dnaberts_embedding(data_loader, tokenizer, model, device, model_max_length=400, embed_dir=None):
    model.eval()
    ...
    for j, batch in enumerate(tqdm.tqdm(data_loader)):
        with torch.no_grad():
            ...
            np.save(os.path.join(embed_dir, f"zs_train_embeddings_batch_{j}.npy"), batch_embeddings)
            np.save(os.path.join(embed_dir, f"zs_train_res_phenotypes_batch_{j}.npy"), res_phenotypes)
